...
describe('Auth (e2e)', () => {
  ...
  describe('register', () => {
    ...
    it('should throw if username exists', async () => {
      ...
      await expect(authService.register('testuser', 'password123')).rejects.toThrow('Username already exists');
    });
  });
  ...
  describe('AuthController', () => {
    it('should call register via controller', async () => {
      ...
      jest.spyOn(authService, 'register').mockResolvedValue({ username: 'testuser', role: 'user' });
    ...
    it('should call login via controller', async () => {
      const req = { user: { _doc: { _id: 'user123', username: 'testuser', role: 'user' } } };
      jest.spyOn(authService, 'login').mockResolvedValue({ access_token: 'mocked.jwt.token' });
      const result = await authController.login(req as any, { username: 'testuser', password: 'password123' });
      expect(result).toEqual({ access_token: 'mocked.jwt.token' });
    });
  });
});